...
<body>
    <div class="header">
        ...
        <div class="header-right">
            <span class="user-info">Ingelogd als: <?php echo e(session('admin_username', 'Admin')); ?></span>
    ...
    <div class="container">
        ...
        <div class="stats">
            <div class="stat-card highlight">
                ...
                <div class="value"><?php echo e($orders->total()); ?></div>
            </div>
            <div class="stat-card">
                <div class="label">Betaald (wacht op print)</div>
                <div class="value"><?php echo e(\App\Models\Order::where('status', 'paid')->count()); ?></div>
            </div>
            <div class="stat-card">
                <div class="label">Verzonden</div>
                <div class="value"><?php echo e(\App\Models\Order::where('status', 'shipped')->count()); ?></div>
            </div>
            <div class="stat-card">
                <div class="label">Omzet totaal</div>
                <div class="value">€ <?php echo e(number_format(\App\Models\Order::where('status', '!=', 'cancelled')->sum('price_total') / 100, 2, ',', '.')); ?></div>
            </div>
        </div>
        ...
        <div class="card" style="margin-bottom: 2rem;">
            <div class="card-header">
                ...
                <form method="GET" action="<?php echo e(route('admin.invoices.monthly')); ?>" target="_blank" style="display: flex; gap: 10px; align-items: center;">
                    <select name="month" style="padding: 6px 10px; border: 2px solid #e9ecef; border-radius: 8px; font-family: inherit; font-size: 14px;">
                        <?php $__currentLoopData = ['januari','februari','maart','april','mei','juni','juli','augustus','september','oktober','november','december']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $maand): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($i + 1); ?>" <?php echo e((int)now()->month === $i + 1 ? 'selected' : ''); ?>><?php echo e(ucfirst($maand)); ?></option>
        ...
        <div class="card">
            ...
            <?php if($orders->count() > 0): ?>
                ...
                <form id="bulkDeleteForm" method="POST" action="<?php echo e(route('admin.orders.bulk-delete')); ?>">
                    <?php echo csrf_field(); ?>
                <table>
                    <thead>
                        <tr>
                        ...
                        <?php $__currentLoopData = $orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr>
                                <td class="checkbox-cell">
                                    <input type="checkbox" name="order_numbers[]" value="<?php echo e($order->order_number); ?>" class="order-checkbox" onchange="updateBulkBar()">
                                </td>
                                <td>
                                    <span class="order-number"><?php echo e($order->order_number); ?></span>
                                </td>
                                <td>
                                    <div class="customer-name"><?php echo e($order->customer_name); ?></div>
                                    <div class="customer-email"><?php echo e($order->customer_email); ?></div>
                                    <?php if($order->utm_source || $order->gclid || $order->fbclid): ?>
                                    <div class="customer-email" style="color: #6366f1;"><?php echo e($order->attribution_summary); ?></div>
                                    <?php endif; ?>
                                </td>
                                <td class="file-info">
                                    <span class="file-name" title="<?php echo e($order->pdf_original_name); ?>"><?php echo e($order->pdf_original_name); ?></span>
                                    <div class="file-meta">
                                        <?php echo e($order->format); ?> • <?php echo e($order->page_count); ?> pag. • <?php echo e($order->quantity ?? 1); ?>× •
                                        <?php if($order->binding_type === 'booklet'): ?>
                                            Geniet
                                        <?php else: ?>
                                            Losbladig <?php echo e($order->print_side === 'double' ? '(2-z)' : '(1-z)'); ?>
                                ...
                                <td class="price">€ <?php echo e(number_format($order->price_total / 100, 2, ',', '.')); ?></td>
                                <td>
                                    <span class="status <?php echo e($order->status); ?>">
                                        <?php switch($order->status):
                                            case ('pending'): ?> In afwachting <?php break; ?>
                                            <?php case ('paid'): ?> Betaald <?php break; ?>
                                            <?php case ('processing'): ?> In productie <?php break; ?>
                                            <?php case ('shipped'): ?> Verzonden <?php break; ?>
                                            <?php case ('delivered'): ?> Afgeleverd <?php break; ?>
                                            <?php case ('cancelled'): ?> Geannuleerd <?php break; ?>
                                ...
                                <td class="actions">
                                    <?php if($order->pdf_path): ?>
                                        <a href="/admin/orders/<?php echo e($order->order_number); ?>/pdf" class="btn btn-secondary" title="Download originele PDF">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    ...
                                    <?php endif; ?>
                                    <?php if($order->binding_type === 'booklet' && $order->status !== 'pending' && $order->status !== 'cancelled'): ?>
                                        <a href="/admin/orders/<?php echo e($order->order_number); ?>/imposed" class="btn btn-primary" title="Download inslag PDF (drukklaar)">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    ...
                                    <?php endif; ?>
                                    <a href="/admin/orders/<?php echo e($order->order_number); ?>/pakbon" class="btn btn-secondary" target="_blank" title="Pakbon">
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                                            <polyline points="14 2 14 8 20 8"/>
                                            <line x1="16" y1="13" x2="8" y2="13"/>
                                    ...
                                    <?php if($order->isPaid()): ?>
                                        <?php $invoice = \App\Models\Invoice::where('order_id', $order->id)->first(); ?>
                                        <?php if($invoice): ?>
                                            <a href="/admin/orders/<?php echo e($order->order_number); ?>/invoice" class="btn btn-secondary" target="_blank" title="Download factuur <?php echo e($invoice->invoice_number); ?>">
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                                                    <polyline points="14 2 14 8 20 8"/>
                                            ...
                                            <button type="button" class="btn btn-secondary" onclick="resendInvoice('<?php echo e($order->order_number); ?>', '<?php echo e($invoice->invoice_number); ?>', '<?php echo e($order->customer_email); ?>')" title="Factuur nogmaals mailen naar <?php echo e($order->customer_email); ?>">
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                        ...
                                        <?php else: ?>
                                            <button type="button" class="btn btn-secondary" onclick="createInvoice('<?php echo e($order->order_number); ?>')" title="Factuur aanmaken voor deze bestelling" style="border: 1px dashed #6c757d;">
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                                                    <polyline points="14 2 14 8 20 8"/>
                                    ...
                                    <?php if($order->status === 'paid'): ?>
                                        <?php if($order->delivery_type === 'pickup'): ?>
                                            <form method="POST" action="/admin/orders/<?php echo e($order->order_number); ?>/ship" style="display:inline;">
                                                <?php echo csrf_field(); ?>
                                        ...
                                        <?php else: ?>
                                            <button type="button" class="btn btn-success" onclick="openShipModal('<?php echo e($order->order_number); ?>')" title="Markeer als verzonden">
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <?php if($orders->hasPages()): ?>
                    <div class="pagination">
                        <?php if($orders->onFirstPage()): ?>
                            <span>← Vorige</span>
                        <?php else: ?>
                            <a href="<?php echo e($orders->previousPageUrl()); ?>">← Vorige</a>
                        <?php endif; ?>
                        ...
                        <?php if($orders->hasMorePages()): ?>
                            <a href="<?php echo e($orders->nextPageUrl()); ?>">Volgende →</a>
                        <?php else: ?>
                            <span>Volgende →</span>
                        <?php endif; ?>
                    </div>
...
<?php /**PATH /Users/elcoroest/Documents/GitHub/printmijnpdf/resources/views/admin/orders.blade.php ENDPATH**/ ?>