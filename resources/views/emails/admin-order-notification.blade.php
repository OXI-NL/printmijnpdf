...
<head>
    ...
    <title>Orderbon - {{ $order->order_number }}</title>
    <!--[if mso]>
    <style type="text/css">
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width: 580px; background: white; border-radius: 20px; overflow: hidden; box-shadow: 0 8px 40px rgba(0,0,0,0.08);">
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td>
                                        ...
                                        <span style="color: white; font-size: 24px; font-weight: 700; font-family: 'SF Mono', Monaco, 'Courier New', monospace; letter-spacing: 0.5px;">{{ $order->order_number }}</span>
                                    </td>
                                    <td style="text-align: right;">
                                        <span style="color: rgba(255,255,255,0.8); font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">Totaal</span><br>
                                        <span style="color: white; font-size: 24px; font-weight: 700;">{{ $order->formatted_total }}</span>
                                    </td>
                                </tr>
                            </table>
                        </td>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td style="padding: 8px 12px; background: #f0fdf4; border-radius: 8px; text-align: center;">
                                        <span style="color: #166534; font-size: 13px; font-weight: 600;">Betaald op {{ $order->paid_at ? $order->paid_at->format('d-m-Y \o\m H:i') : $order->created_at->format('d-m-Y \o\m H:i') }}</span>
                                    </td>
                                </tr>
                            </table>
                        </td>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
                                <tr>
                                    ...
                                    <td style="padding: 12px 16px; background: #f8fafc; border-bottom: 1px solid #e2e8f0;">
                                        <span style="color: #1e293b; font-size: 13px; font-weight: 600;">{{ $order->customer_name }}</span>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td style="padding: 12px 16px; border-bottom: 1px solid #e2e8f0;">
                                        <a href="mailto:{{ $order->customer_email }}" style="color: #e63946; font-size: 13px; text-decoration: none; font-weight: 500;">{{ $order->customer_email }}</a>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td style="padding: 12px 16px; border-bottom: 1px solid #e2e8f0; background: #f8fafc;">
                                        <a href="tel:{{ $order->customer_phone }}" style="color: #1e293b; font-size: 13px; text-decoration: none;">{{ $order->customer_phone }}</a>
                                    </td>
                                </tr>
                                @endif
                                <tr>
                                    <td style="padding: 12px 16px;{{ $order->customer_phone ? '' : ' background: #f8fafc;' }}">
                    ...
                    <tr>
                        <td style="padding: 20px 30px 0;">
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background: {{ $order->delivery_type === 'pickup' ? '#f0f9ff' : '#fefce8' }}; border-radius: 10px; padding: 14px 16px;">
                                <tr>
                                    <td style="padding: 14px 16px;">
                                        ...
                                        @else
                                            ...
                                            <span style="color: #854d0e; font-size: 13px;">Naar: {{ $order->address_street }} {{ $order->address_number }}{{ $order->address_addition ? ' '.$order->address_addition : '' }}, {{ $order->address_postcode }} {{ $order->address_city }}</span>
                                        @endif
                                    </td>
                                </tr>
                            </table>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
                                <tr>
                                    ...
                                    <td style="padding: 12px 16px; background: #f8fafc; border-bottom: 1px solid #e2e8f0; text-align: right;">
                                        <span style="color: #1e293b; font-size: 13px; font-weight: 600;">{{ $order->pdf_original_name }}</span>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding: 12px 16px; border-bottom: 1px solid #e2e8f0;">
                                ...
                                <tr>
                                    ...
                                    <td style="padding: 12px 16px; border-bottom: 1px solid #e2e8f0; text-align: right;">
                                        <span style="color: #1e293b; font-size: 13px; font-weight: 600;">
                                            ...
                                            @else
                                                Losse pagina's ({{ $order->print_side === 'double' ? 'dubbelzijdig' : 'enkelzijdig' }})
                                            @endif
                                        </span>
                                    </td>
                                </tr>
                    ...
                    <tr>
                        <td style="padding: 25px 30px 0;">
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
                                ...
                                @endif
                                <tr>
                                    <td style="padding: 10px 16px;{{ $order->binding_type !== 'booklet' ? ' background: #f8fafc;' : '' }} border-bottom: 1px solid #e2e8f0;">
                                        <span style="color: #64748b; font-size: 13px;">{{ $order->delivery_type === 'pickup' ? 'Afhalen' : 'Verzendkosten (PostNL)' }}</span>
                                    </td>
                                    <td style="padding: 10px 16px;{{ $order->binding_type !== 'booklet' ? ' background: #f8fafc;' : '' }} border-bottom: 1px solid #e2e8f0; text-align: right;">
                                        <span style="color: #1e293b; font-size: 13px;">
                                            @if($order->price_shipping > 0)
                                                &euro; {{ number_format($order->price_shipping / 100, 2, ',', '.') }}
                                            @else
                                                Gratis
                                            @endif
                                        </span>
                                ...
                                <tr>
                                    ...
                                    <td style="padding: 14px 16px; background: #1e293b; text-align: right;">
                                        <span style="color: white; font-size: 18px; font-weight: 700;">{{ $order->formatted_total }}</span>
                                    </td>
                                </tr>
                            </table>
                        </td>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td style="text-align: center; padding: 16px; background: #f8fafc; border-radius: 12px;">
                                        <a href="{{ url('/admin/orders') }}" style="color: #e63946; font-size: 14px; font-weight: 600; text-decoration: none;">Bekijk in admin panel &rarr;</a>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    ...
                    <tr>
                        <td bgcolor="#1e293b" style="background: #1e293b; padding: 20px 30px; text-align: center;">
                            <p style="color: #475569; font-size: 12px; margin: 0;">
                                Automatisch gegenereerde orderbon &middot; {{ now()->format('d-m-Y H:i') }}
                            </p>
                        </td>
                    </tr>
